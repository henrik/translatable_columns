  ...
  module ClassMethods
    ...
    def define_translated_getter_with_defaults(column)
      define_method column do
        this_locale = self.send(self.class.column_translated(column))
        return this_locale unless this_locale.blank?

        default_locale = self.send(self.class.column_name_localized(column, I18n.default_locale))
        return default_locale unless default_locale.blank?

        self.find_translated_value_for(column)  # any locale
      end
    end
